...
namespace SWS
{
    ...
    public class splineMove : MonoBehaviour
    {
        ...
        void Start()
        {
            pathContainer = GameObject.Find("way01").GetComponent<PathManager>();
            if (onStart)
                StartMove();
        }
        ...
        private void CreateTween()
        {
            ...
            if (local)
            {
                tween = transform.DOLocalPath(wpPos, originSpeed, pathType, pathMode)
                                 ...
                                 .SetLookAt(lookAhead, forwardDir, faceDir);
            }
            else
            {
        ...
        private void FixFaceDir()
        {
            if (CheckPoint.Length != 0)
            {
                ...
                if (Physics.Raycast(CheckPoint[0].transform.position, CheckPoint[0].transform.forward, out tempOneHit, 3, HitMask))
                {
                    RaycastHit tempTwoHit;
                    if (Physics.Raycast(CheckPoint[1].transform.position, CheckPoint[1].transform.forward, out tempTwoHit, 3, HitMask))
                    {
                        RaycastHit tempThreeHit;
                        if (Physics.Raycast(CheckPoint[2].transform.position, CheckPoint[2].transform.forward, out tempThreeHit, 3, HitMask))
                        {
                            //-----1-2为正   2-1为负
                            Vector3 wayTempOne = (tempOneHit.point - tempTwoHit.point).normalized;
                            Vector3 wayTempTwo = (tempOneHit.point - tempThreeHit.point).normalized;
                            Vector3 tempDir = Vector3.Cross(wayTempOne, wayTempTwo);
                            //--求得汽车所在面
                            Vector3 carPosDir = (CheckPoint[0].transform.position - tempOneHit.point).normalized;
                            float angle = Vector3.Angle(carPosDir, tempDir);
                            faceDir = angle>90f?-tempDir : tempDir;
                            Debug.DrawLine(CheckPoint[0].transform.position, CheckPoint[0].transform.position + faceDir * 2, Color.red, 3f);
                        }
            ...
            forwardDir = transform.position - recordPos;
            transform.rotation = Quaternion.LookRotation(forwardDir, faceDir);
